...
function OwnerWorkspace({ characters, onClose, onRefresh }: { characters: Character[]; onClose: () => void; onRefresh: () => void }) {
  ...
  const addNotification = trpc.owner.addNotification.useMutation({ onSuccess: () => { toast.success("Đã gửi thông báo tới đồng cỏ."); setNoticeTitle("Một lời nhắn từ đồng cỏ"); setNoticeBody(""); setNoticePublishedAt(""); setNoticePinned(false); } });
